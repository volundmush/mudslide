...
- Lifted from Evennia develop branch 7/18/2020
- Volund

"""
from weakref import WeakKeyDictionary
from django.utils.translation import gettext as _
from honahlee.utils.misc import inherits_from, is_iter
...
class CmdSet(object, metaclass=_CmdSetMeta):
    ...
    # A dict of <string, <bool/none>> that define what kind of 'types' this cmdset contains. This might be 'channels' or 'exits'
    # for instance. This is intended for filtering purposes.
    cmdset_types = dict()

    # A tuple of strings for what cmdset command types contained in lower-priority will be filtered out.
    ...
    def __init__(self, cmdsetobj=None, key=None):
        ...
        self.merged_filter_types = dict(self.cmdset_filter_types)
    ...
    def __add__(self, cmdset_a):
        ...
        if self.priority <= cmdset_a.priority:
            ...
            higher = cmdset_a
            lower = self
        sys_commands_higher = higher.get_system_cmds()
        sys_commands_lower = lower.get_system_cmds()

        # preserve system __commands
        sys_commands = sys_commands_higher + [
            cmd for cmd in sys_commands_lower if cmd not in sys_commands_higher
        ]

        mergetype = higher.key_mergetypes.get(lower.key, higher.mergetype)
        if mergetype == "Intersect":
            cmdset_c = self._intersect(higher, lower)
        elif mergetype == "Replace":
            cmdset_c = self._replace(higher, lower)
        elif mergetype == "Remove":
            cmdset_c = self._remove(higher, lower)
        else:  # Union
            cmdset_c = self._union(higher, lower)
        ...
        cmdset_c.merged_filter_types.update(lower.merged_filter_types)
        cmdset_c.merged_filter_types.update(higher.merged_filter_types)
    ...
    def add(self, cmd, allow_duplicates=False):
        ...
        if inherits_from(cmd, "honahlee.utils.cmdset.CmdSet"):
